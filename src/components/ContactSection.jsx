 ...
 */
import { Mail, Phone, MapPin, Send, Github, Linkedin, Instagram, ArrowUpRight, CheckCircle2 } from "lucide-react";
...
const VALIDATORS = { name: v => v.trim().length >= 2, email: v => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v), subject: v => v.trim().length >= 3, message: v => v.trim().length >= 10 };
const ERRORS = { name: "Please enter your name (min 2 chars).", email: "Please enter a valid email address.", subject: "Please enter a subject.", message: "Message must be at least 10 characters." };
...
function InfoCard({ Icon, label, value, href, accent }) {
  ...
  return (
    <Tag href={href || undefined} className="c-info-card"
      onMouseEnter={e => { e.currentTarget.style.borderColor = `${accent}50`; e.currentTarget.querySelector(".c-info-icon").style.background = `${accent}15`; e.currentTarget.querySelector(".c-info-icon").style.borderColor = `${accent}40`; }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.querySelector(".c-info-icon").style.background = "rgba(255,255,255,0.04)"; e.currentTarget.querySelector(".c-info-icon").style.borderColor = "var(--border)"; }}>
...
function FormField({ label, id, name, type, placeholder, value, onChange, onBlur, error, touched, rows }) {
  ...
  const hasError = touched && error; const isValid = touched && !error && value.length > 0;
  const cls = `form-input${hasError ? " error" : isValid ? " valid" : ""}`;
  return (
    <div>
      <label htmlFor={id} style={{ display: "block", fontFamily: "'JetBrains Mono',monospace", fontSize: "0.62rem", letterSpacing: "0.12em", textTransform: "uppercase", color: hasError ? "#ef4444" : "var(--text-muted)", marginBottom: "0.4rem" }}>{label}</label>
      {type === "textarea"
      ...
      {hasError && <p style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.6rem", letterSpacing: "0.06em", color: "#ef4444", marginTop: "0.4rem" }}>{error}</p>}
    </div>
  );
}
...
function SuccessState({ onReset }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "3rem 1rem", gap: "1rem" }}>
      ...
      <p style={{ fontFamily: "'Outfit',sans-serif", fontSize: "0.88rem", color: "var(--text-muted)", maxWidth: 280, lineHeight: 1.65 }}>Thanks for reaching out. I'll get back to you within 24 hours.</p>
      <button onClick={onReset} style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.68rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent)", background: "none", border: "none", cursor: "pointer", marginTop: "0.5rem", display: "flex", alignItems: "center", gap: "0.4rem", transition: "opacity 0.2s" }} onMouseEnter={e => { e.currentTarget.style.opacity = "0.65"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "1"; }}>← Send another</button>
    </div>
  );
}
...
export default function ContactSection() {
  ...
  const [form, setForm] = useState(INIT_FORM); const [errors, setErrors] = useState(INIT_ERRORS); const [touched, setTouched] = useState(INIT_TOUCHED); const [sending, setSending] = useState(false); const [sent, setSent] = useState(false);
  const validateField = (name, value) => VALIDATORS[name](value) ? "" : ERRORS[name];
  const handleChange = e => { const { name, value } = e.target; setForm(f => ({ ...f, [name]: value })); if (touched[name]) setErrors(er => ({ ...er, [name]: validateField(name, value) })); };
  const handleBlur = e => { const { name, value } = e.target; setTouched(t => ({ ...t, [name]: true })); setErrors(er => ({ ...er, [name]: validateField(name, value) })); };
  const handleSubmit = e => { e.preventDefault(); const newErrors = Object.fromEntries(Object.keys(form).map(k => [k, validateField(k, form[k])])); setTouched({ name: true, email: true, subject: true, message: true }); setErrors(newErrors); if (Object.values(newErrors).some(Boolean)) return; setSending(true); setTimeout(() => { setSending(false); setSent(true); }, 1800); };
  const handleReset = () => { setForm(INIT_FORM); setErrors(INIT_ERRORS); setTouched(INIT_TOUCHED); setSent(false); };
  const reveal = (d = 0, axis = "Y") => ({ opacity: secVisible ? 1 : 0, transform: secVisible ? "translate(0,0)" : axis === "X-" ? "translateX(-24px)" : axis === "X+" ? "translateX(24px)" : "translateY(24px)", transition: `opacity .75s ${d}s ease,transform .75s ${d}s ease` });

  return (
    <>
      <style>{CONTACT_STYLES}</style>
      <section id="contact" ref={secRef} style={{ position: "relative", padding: "8rem 1.5rem", background: "var(--bg)", overflow: "hidden", fontFamily: "'Outfit',sans-serif" }}>
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "3rem" }} className="lg:grid-cols-[1fr_1.15fr]">
            ...
            <div className="form-card" style={reveal(0.32, "X+")}>
              ...
              {sent ? <SuccessState onReset={handleReset} /> : (
                <>
                  ...
                  <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
                    ...
                    <button type="submit" disabled={sending} className="submit-btn" style={{ marginTop: "0.5rem" }}>
                      {sending ? (<><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: "spinLoader 0.8s linear infinite" }}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" /></svg>Sending...</>) : (<>Send Message<Send size={13} /></>)}
                    </button>
                  </form>
                </>
              )}
